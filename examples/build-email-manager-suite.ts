...
/**
 * Email Manager Suite Builder
 *
 * This script builds the email-manager suite package and all its dependencies
 ...
 * - Builds 19+ packages in dependency order
 ...
 *
 * Run with: npx tsx examples/build-email-manager-suite.ts
 */
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║        Email Manager Suite Builder                         ║');
  console.log('╚════════════════════════════════════════════════════════════╝\n');

  console.log('📦 Building email-manager suite from plan file');
  console.log('📁 Plan: plans/packages/suite/email-manager.md');
  console.log('🤖 Using PackageBuilderWorkflow with planPath mode');
  console.log('🔗 Will parse and build 19+ dependencies automatically\n');
  ...
  try {
    ...
    const buildId = `email-manager-suite-${Date.now()}`;
    const workflowId = `build-email-manager-${Date.now()}`;
    const workspaceRoot = '/Users/mattbernier/projects/tools';

    const input: PackageBuilderInput = {
      buildId,
      workspaceRoot,
      planPath: 'plans/packages/suite/email-manager.md', // 👈 Direct plan path!
      config: {
    ...
    console.log('📋 Step 4: Building Email Manager Suite');
    console.log('   This will:');
    console.log('   1. Parse email-manager.md plan file');
    console.log('   2. Extract all 19+ package dependencies');
    ...
    try {
      ...
      console.log('\n════════════════════════════════════════════════════════════');
      console.log('🎉 Email Manager Suite Build Complete!\n');
      console.log('📊 Results:');

-- ============================================================================
-- Migration: Create project_connectors table for cross-project communication
-- Created: 2025-01-20
-- Description: Stores connectors for cross-project service communication via Temporal Nexus
-- ============================================================================

CREATE TABLE IF NOT EXISTS project_connectors (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  source_project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  target_project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  target_service_id UUID NOT NULL REFERENCES workflows(id) ON DELETE CASCADE,
  target_interface_id UUID NOT NULL REFERENCES service_interfaces(id) ON DELETE CASCADE,
  name VARCHAR(255) NOT NULL,
  display_name VARCHAR(255) NOT NULL,
  description TEXT,
  nexus_endpoint_name VARCHAR(255) NOT NULL, -- Nexus endpoint identifier
  nexus_service_name VARCHAR(255) NOT NULL, -- Nexus service name
  visibility VARCHAR(50) DEFAULT 'private' CHECK (visibility IN ('private', 'public', 'organization')),
  auth_config JSONB, -- Auth configuration if public
  encrypted_auth_config BYTEA, -- Encrypted auth config if needed
  encryption_key_id VARCHAR(255),
  created_by UUID NOT NULL REFERENCES users(id),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE(source_project_id, name)
);

-- Indexes for performance
CREATE INDEX idx_project_connectors_source ON project_connectors(source_project_id);
CREATE INDEX idx_project_connectors_target ON project_connectors(target_project_id);
CREATE INDEX idx_project_connectors_target_service ON project_connectors(target_service_id);
CREATE INDEX idx_project_connectors_target_interface ON project_connectors(target_interface_id);
CREATE INDEX idx_project_connectors_nexus_endpoint ON project_connectors(nexus_endpoint_name);
CREATE INDEX idx_project_connectors_nexus_service ON project_connectors(nexus_service_name);

-- RLS Policies
ALTER TABLE project_connectors ENABLE ROW LEVEL SECURITY;

-- Policy: Users can view connectors for their own projects
CREATE POLICY "Users can view own project connectors"
  ON project_connectors
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = project_connectors.source_project_id
      AND projects.created_by = auth.uid()
    )
  );

-- Policy: Users can create connectors for their own projects
CREATE POLICY "Users can create own project connectors"
  ON project_connectors
  FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = project_connectors.source_project_id
      AND projects.created_by = auth.uid()
    )
    AND
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = project_connectors.target_project_id
      AND projects.created_by = auth.uid()
    )
  );

-- Policy: Users can update connectors for their own projects
CREATE POLICY "Users can update own project connectors"
  ON project_connectors
  FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = project_connectors.source_project_id
      AND projects.created_by = auth.uid()
    )
  );

-- Policy: Users can delete connectors for their own projects
CREATE POLICY "Users can delete own project connectors"
  ON project_connectors
  FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM projects
      WHERE projects.id = project_connectors.source_project_id
      AND projects.created_by = auth.uid()
    )
  );

-- Function to update updated_at timestamp
CREATE OR REPLACE FUNCTION update_project_connectors_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Trigger to automatically update updated_at
CREATE TRIGGER update_project_connectors_updated_at
  BEFORE UPDATE ON project_connectors
  FOR EACH ROW
  EXECUTE FUNCTION update_project_connectors_updated_at();


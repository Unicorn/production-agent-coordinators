...
export async function gitCommit(input) {
    ...
    try {
        ...
        if (gitUser) {
            await execPromise(`git config user.name "${gitUser.name}"`, { cwd: workspacePath });
            await execPromise(`git config user.email "${gitUser.email}"`, { cwd: workspacePath });
        }
        ...
        const { stdout: statusOut } = await execPromise('git status --porcelain', { cwd: workspacePath });
        if (!statusOut.trim()) {
            return {
                success: true,
        ...
        // Commit with message
        const { stdout } = await execPromise(`git commit -m "${message.replace(/"/g, '\\"')}"`, { cwd: workspacePath });
        // Get commit hash
        const { stdout: hashOut } = await execPromise('git rev-parse HEAD', { cwd: workspacePath });
        return {
            success: true,
            duration: Date.now() - startTime,
...
export async function gitPush(input) {
    ...
    try {
        ...
        const { stdout: statusOut } = await execPromise(`git status -sb`, { cwd: workspacePath });
        console.log(`[GitPush] Status: ${statusOut.trim()}`);
        // Push to remote
        const forceFlag = force ? '--force' : '';
        const { stdout, stderr } = await execPromise(`git push ${forceFlag} ${remote} ${branch}`.trim(), { cwd: workspacePath });
        return {
            success: true,
            duration: Date.now() - startTime,
            stdout,
 ...
 */
export async function gitCreateBranch(input) {
    ...
    try {
        ...
        const { stdout, stderr } = await execPromise(`git checkout -b ${branchName}`, { cwd: workspacePath });
        return {
            success: true,
            stdout,
            stderr
 ...
 */
export async function gitCreatePR(input) {
    const { workspacePath, branch, title, body, baseBranch = 'main', draft = false, labels = [] } = input;
    try {
...
export async function createWorktree(input) {
    ...
    try {
        ...
        const { stdout, stderr } = await execPromise(`git worktree add ${worktreePath} ${baseBranchFlag}`, { cwd: repoPath });
        ...
        if (removeBranches && branchNamesToRemove.length > 0) {
            for (const branchName of branchNamesToRemove) {
                try {
                    ...
                    const { stdout } = await execPromise(`git branch --list ${branchName}`, { cwd: mainWorkspace });
                    if (stdout.trim()) {
...
//# sourceMappingURL=git.activities.js.map
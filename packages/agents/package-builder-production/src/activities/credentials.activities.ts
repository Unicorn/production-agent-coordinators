...
export interface CredentialsStatus {
  ...
  checks: Record<string, CredentialCheckResult> | CredentialCheckResult[]; // Support both formats
  missing: string[];
}
 ...
 */
export async function checkGitHubCLI(): Promise<CredentialCheckResult> {
  try {
 ...
 */
export async function checkNPM(): Promise<CredentialCheckResult> {
  try {
 ...
 */
export async function checkPackagesAPI(): Promise<CredentialCheckResult> {
 ...
 */
export async function checkGit(): Promise<CredentialCheckResult> {
  try {
 ...
 */
export async function checkClaudeCLI(): Promise<CredentialCheckResult> {
  try {
 ...
 */
export async function checkGeminiCLI(): Promise<CredentialCheckResult> {
  try {
...
export async function checkCredentials(
  ...
  const {
    checkGitHub: shouldCheckGitHub = true,
    checkNPM: shouldCheckNPM = true,
    checkPackagesAPI: shouldCheckPackagesAPI = true,
    checkGit: shouldCheckGit = true,
    checkClaude: shouldCheckClaude = false,
    checkGemini: shouldCheckGemini = false
  } = input;

  const checks: CredentialCheckResult[] = [];

  // Run all checks in parallel
  const checkPromises: Promise<CredentialCheckResult>[] = [];
  ...
  const checksObj: Record<string, CredentialCheckResult> = {};
 ...
 */
export function formatCredentialsError(status: CredentialsStatus): string {

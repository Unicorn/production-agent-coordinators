...
// Priority: 1) System env, 2) .env file (dev/staging only), 3) Hard fail
try {
  loadDotenv(); // Loads .env if it exists, does nothing if not
} catch (error) {
  // .env file not found or invalid - that's OK if system env is set
}

// Validate required environment variables
const requiredEnvVars = ['NPM_TOKEN'];
...
if (missingEnvVars.length > 0) {
  ...
  console.error('');
  console.error('Set them in your system environment or create a .env file with:');
  missingEnvVars.forEach(varName => {
    console.error(`   ${varName}=your_${varName.toLowerCase()}_here`);
  });
  console.error('');
  console.error('SECURITY NOTE: .env files should only be used in dev/staging environments.');
  console.error('Production environments MUST use system environment variables.');
  process.exit(1);
}
...
async function main() {
  ...
  config.npmToken = process.env.NPM_TOKEN;
